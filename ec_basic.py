...
def prepare_run(entity, project, args, folder_name="results"):
    ...
    folder = Path(folder_name) / today / run.project / run.name
...
if __name__ == "__main__":
    ...
    run, folder = prepare_run(entity="molanu", project=args.proj, args)

    defaults = deepcopy(args)

    for rep in range(5):
        for num_genes_consider in [0.2, 0.5, 0.8]:
            args = deepcopy(defaults)
            args.num_genes_consider = num_genes_consider
            print(args)
            evolutionary_algorithm(args, f"{num_genes_consider}_{rep}_", folder)
